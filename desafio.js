   ...
   //############################################################
    ...
    function descifrar (alfabeto, pos) {
        ...
        for (x = 0; x < letras.length; x++){
            lugar = pos[alfabeto[letras[x]]]; 
            largo_lugar = lugar.length;
            for (n=0; n < largo_lugar ; n++) {
               cadena[lugar[n]] = letras[x];
            }
        }
    ...
    // Aca realizo una prueba, codificando una cadena para ver si luego la retorna igual.
    // #####################################################################################
    ...
    for (orden = 0; orden < cadena_de_prueba.length; orden++){
      lista_dir=[] 
      letra = cadena_de_prueba [orden]
      if (prueba_alfabeto[letra] == undefined) {
          prueba_alfabeto[letra] =  "*" +letra + "*" ;  // Como codigo usare la letra entre dos "*" (para simplificar el algoritmo y no usar funciones como time u random)
          lista_dir.push(orden);
          prueba_posiciones["*" + letra + "*"] = lista_dir;
      } else {
        lista_dir = prueba_posiciones["*" + letra + "*"];
        lista_dir.push(orden);
        prueba_posiciones ["*" + letra + "*"] = lista_dir;
        }
    }
