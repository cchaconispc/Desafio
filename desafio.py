...
def descifrar (alfabeto, pos):
  ...
  for letra in alfabeto:
      lugares=pos[alfabeto[letra]]
      for lugar in lugares:
        cadena[lugar]=letra
  direccion = "".join(cadena)
  return (direccion)   #Retorno el resultado en forma de String para ser usado luego.
...
for letra in cadena_de_prueba
  ...
  if letra not in alfabeto_prueba:
      ...
      lista_dir.append(orden)
      pos_prueba[letra * 5] = lista_dir
  else:
    lista_dir= pos_prueba[letra * 5]
    lista_dir.append(orden)
    pos_prueba [letra * 5] = lista_dir
  orden += 1
